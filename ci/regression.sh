...
# Boundary-check scripts ship next to this script in both the source tree
# and the build tree; resolve them relative to THIS file, not /home/ubuntu/grxgpu,
# because the build tree can run on a different host than the one that ran
# configure (e.g. artifact downloaded onto a CI runner).
"$(dirname "${BASH_SOURCE[0]}")/check_config_boundary.sh"

# Enforce the sw/ ↔ sim/+hw/ bidirectional isolation boundary.
# See AGENTS.md §6 and docs/coding_guidelines_cpp.md §8.
"$(dirname "${BASH_SOURCE[0]}")/check_sw_sim_boundary.sh"
