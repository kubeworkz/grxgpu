...
- **Separate pre-existing finding surfaced by the A/B:** under rtlsim (real TFR RTL), the GEMM's ULP check (`FLOAT_ULP=12`) fails at K≥256 — relative error ~1.5e-6 at K=256, ~3.2e-6 at K=512, uniform across elements — while simx (C++ TCU model) passes the same test at K=512. This is fp32 accumulation drift in the TFR (each multiply-add rounds) vs the test's double-precision reference; it is *independent of DATA_OOB* (bit-identical in both builds) and is a test-tolerance/model-fidelity question, not an RTL functional bug. Options for the tapeout track: widen the GEMM tolerance for K≥256, or match the reference accumulation semantics to the TFR.
