...
- **The L2 request bus is 91% write-data plane** (`syn/l2_req_bus_analysis.py`, traced from the BB16 netlist). `VX_mem_bus_arb`'s `REQ_DATAW = 1 + ADDR_WIDTH + DATA_WIDTH + DATA_SIZE + ATTR_WIDTH + TAG_WIDTH` resolves to **630 bits**: rw(1) + addr(26) + **data(512)** + **byteen(64)** + attr(17) + tag(10). The control plane (rw+addr+attr+tag) is just **54 bits (8.6%)**; the data plane (data+byteen) is **576 bits (91.4%)** and is only exercised on writes. For read-dominated traffic (GEMM/DXA), the arbiter never needs the data plane at all.
- **Control-plane-only arbiter — implemented and measured** (`DATA_OOB` param in `hw/rtl/mem/VX_mem_bus_arb.sv`): with `DATA_OOB=1` the request arbiter carries only the 54b control plane through `VX_stream_arb`; the 576b write-data plane follows out-of-band through a parallel bufferless `VX_stream_switch` driven by the same grant `sel_out`. Standalone at the L2 shape (32→2 × 630b, `syn/oob_arb_sweep.sh`): **349,721 → 128,165 µm² (−63.4%)** — less than the −91% bus-composition estimate because the data mux still exists in the switch; what is removed is the arbiter's per-bit grant/priority logic and elastic buffering on the 576b payload.
- **Measured cluster fabric win: −5.9%** (`syn/patch_oob_oldtree.py` + BB16 re-synthesis). Re-synthesizing the full 16-socket fabric with `DATA_OOB=1` on *all* `VX_mem_bus_arb` instances gives **10,754,733 µm² vs 11,428,017 µm² baseline (−673,285 µm²)**. Enabling it only on the L2 cache `mem_arb` alone is a wash (+0.004%) — the big 630b arbiters (L2 core-bus NC arb, L3 bypass) are where the win lives. For the G100 this is ~5.4 mm² of fabric per 8 clusters @45nm (~2.1 mm² @28nm) that moves out of the arbitration network. Caveat: the write-data plane still needs a physical path to the bank; this measurement moves it out of the *arbitration* network into a plain routing switch, which is why the cluster win (5.9%) is much smaller than the standalone arb win (63%).
- **Fused A+B pair is now RTL-backed (Sept 2026)** — the fused `vx_dxa_issue_2d_wg_pair` was originally implemented **in simx only**; the RTL DXA had no pair support, so on the Verilated rtlsim driver the pair's rs2 B-tile fields were misread as extra coordinates and the DXA double-buffered WGMMA kernel (which issues a pair every stage) fetched garbage. The fix (`hw/rtl/dxa/VX_dxa_unit.sv` + `sim/simx/dxa/dxa_core.cpp`) unifies simx and RTL on a **two-sibling-release contract**: a fused pair is split into two independent single-tile transfers, each releasing the barrier once, so a barrier armed with `expect_tx(2)` still opens only when both A and B are resident — identical timing to the old one-release contract. Kernel `expect_tx(1)→(2)` and the TGM FSM `event_attach(1)→(2)` were updated to match. **Verified on rtlsim (1-cluster × 4-core):** DB kernel **K=64 PASS** (69,056 instrs, 48,349 cycles, IPC 1.428) and **K=512 PASS** (483,008 instrs, 277,492 cycles, IPC 1.741) — **bit-identical to simx**. The DB kernel now runs on RTL exactly as the simulator predicts. Build note: the rtlsim Makefile pins `-O1` to dodge a Verilator `V3FuncOpt` crash the new RTL triggers at default `-O2` (a known flaky Verilator bug; `-O0` trips a separate csa_tree codegen bug).
