...
namespace gfx_sw {
// debug hook: device printf (C linkage; libvortex2.a)
...
static inline __attribute__((always_inline)) void tex_sample_f32_layer(
  ...
  vx_printf("SWTEX u=%d v=%d lod=%u fmt=%u w=%u h=%u base=%llx logdim=%x filter=%x wrap=%x\n",
          u, v, lod, s.format, s.width, s.height, (unsigned long long)s.base, s.logdim, filter, s.wrap);
