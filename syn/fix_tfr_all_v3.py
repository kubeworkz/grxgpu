...
# Be careful: don't replace in localparam/parameter NAME positions
CONSTANTS = {
...
# First, replace localparam/parameter definitions to keep the NAME intact
# e.g., "localparam TCU_MAX_INPUTS = ..." -> "localparam TFR_MAX_INPUTS = 16"
for name, value in CONSTANTS.items():
    content = re.sub(r'(localparam|parameter)\s+' + name + r'\b', rf'\1 TFR_{name[4:]}', content)

# Now replace all remaining TCU_* references in expressions
for name, value in CONSTANTS.items():
    content = re.sub(r'\b' + name + r'\b', value, content)

# Clean up: rename TFR_ params back to simple names for readability
# (they're localparams so they don't conflict)
# Actually, just leave them as TFR_* — they're internal
