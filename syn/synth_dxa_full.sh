...
for mod in VX_dxa_watchdog VX_dxa_completion VX_dxa_dispatch VX_dxa_req_arb VX_dxa_desc_table; do
    echo "=== $mod ==="
    $SYNLIG -p "read_systemverilog $DEFS -I. $PKGS ${mod}.sv; hierarchy -top $mod; synth_ecp5 -top ${mod}; stat -tech cpg56f" 2>&1 | grep -E 'Number of (cells|wires)|TRELLIS|EBMF|Slice|DFF|LUT|ERROR|FATAL' | head -20
    echo ""
done
