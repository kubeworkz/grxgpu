...
public:
  ...
  void dtm_set_freg_lane(uint32_t wid, uint32_t reg, uint32_t lane, uint64_t val) {
    operands_.at(wid % VX_CFG_ISSUE_WIDTH)->dtm_freg(wid, reg, lane) = val;
  }
...
void Core::dtm_set_freg_lane(uint32_t wid, uint32_t reg, uint32_t lane, uint64_t val) { impl_->dtm_set_freg_lane(wid, reg, lane, val); }
